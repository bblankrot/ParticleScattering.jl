...
@testset "optimize radius" begin
    ...
    optim_options2 =  Optim.Options(f_tol = 1e-7, iterations = 10,
                            store_trace = true, show_trace = false,
                            allow_f_increases = true)
    res2 = optimize_radius(rs0, r_min, r_max, points, ids, P, θ_i, k0, kin,
    ...
    u1_5 = calc_near_field(k0, kin, 5, sp1, points_, θ_i; opt = fmm_options,
            verbose = false)
    u1_6 = calc_near_field(k0, kin, 6, sp1, points_, θ_i; opt = fmm_options,
            verbose = false)
    u1_7 = calc_near_field(k0, kin, 7, sp1, points_, θ_i; opt = fmm_options,
            verbose = false)
    u2_5 = calc_near_field(k0, kin, 5, sp2, points_, θ_i; opt = fmm_options,
            verbose = false)
    u2_6 = calc_near_field(k0, kin, 6, sp2, points_, θ_i; opt = fmm_options,
            verbose = false)
    u2_7 = calc_near_field(k0, kin, 7, sp2, points_, θ_i; opt = fmm_options,
            verbose = false)
