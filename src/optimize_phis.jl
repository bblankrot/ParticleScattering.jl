function optimize_φ_grad(φs0, points, P, θ_i, k0, kin, shapes, centers, ids, fmmopts, optimopts, minimize, method)
    ...
    if minimize
        # df = OnceDifferentiable(φs -> objF_min(φs, shared_var, initial_φs),
                    # (φs, grad_stor) -> objG!(φs, grad_stor, shared_var, last_φs))
        df = OnceDifferentiable(
            φs -> optimize_φ_grad_f(φs, shared_var, last_φs, α_inc,
                                    H, points, P, θ_i, Ns, k0, centers,
                                    scatteringMatrices, ids, mFMM, fmmopts, buf),
            (grad_stor, φs) -> optimize_φ_grad_g!(grad_stor, φs, shared_var,
...
function optimize_φ_grad_common!(φs, last_φs, shared_var, α_inc, H, points, P, θ_i, Ns, k0, centers, scatteringMatrices, ids, mFMM, opt, buf)
    if (φs != last_φs)
        copy!(last_φs, φs)
...
function optimize_φ_grad_f(φs, shared_var, last_φs, α_inc, H, points, P, θ_i, Ns, k0, centers,scatteringMatrices, ids, mFMM, opt, buf)
    optimize_φ_grad_common!(φs, last_φs, shared_var, α_inc, H, points, P, θ_i, Ns, k0, centers,scatteringMatrices, ids, mFMM, opt, buf)

    func = sum(abs2,shared_var.f)
end

function optimize_φ_grad_g!(grad_stor, φs, shared_var, last_φs, α_inc, H, points, P, θ_i, Ns, k0, centers, scatteringMatrices, scatteringLU, ids, mFMM, opt, buf, minimize)
    optimize_φ_grad_common!(φs, last_φs, shared_var, α_inc, H, points, P, θ_i, Ns, k0, centers,scatteringMatrices, ids, mFMM, opt, buf)
...
#############################################################################3
function optimize_φ(φs0, points, P, θ_i, Ns, k0, kin, shapes, centers, ids, fmmopts, optimopts, minimize::Bool, method)
