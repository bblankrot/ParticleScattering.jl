"""
	minimumN(kout, kin, shape_function; tol = 1e-9, N_points = 10_000, N_start = 400, N_min = 100, N_max = 1_000) -> N, err
...
function minimumN(kout, kin, shape_function; tol = 1e-9, N_points = 10_000, N_start = 400, N_min = 100, N_max = 1_000)
    ...
    E_comp = scatteredField(mu_sigma, kout, s.t, s.ft, s.dft, err_points)
...
"""
	minimumP(k0, kin, s::ShapeParams; tol = 1e-9, N_points = 10_000, P_min = 1, P_max = 60, dist = 2) -> P, errP

Return the minimum `P` necessary to achieve error of at most `tol` in the electric
field, when compared to that obtained with `2N` discretization, for a
