function optimize_radius(rs0, r_min, r_max, points, P, θ_i, k0, kin, centers, fmmopts, optimopts, minimize, method)
    ...
    if minimize
        df = OnceDifferentiable(rs -> optimize_radius_f(rs, last_rs, shared_var, φs, α_inc, H, points, P, θ_i, Ns, k0, kin, centers,scatteringMatrices, dS_S, ids, mFMM, fmmopts, buf),
                    (grad_stor, rs) -> optimize_radius_g!(grad_stor, rs, last_rs, shared_var, φs, α_inc, H, points, P, θ_i, Ns, k0, kin, centers, scatteringMatrices, dS_S, ids, mFMM, fmmopts, buf, minimize))
    else
        df = OnceDifferentiable(rs -> -optimize_radius_f(rs, last_rs, shared_var, φs, α_inc, H, points, P, θ_i, Ns, k0, kin, centers,scatteringMatrices, dS_S, ids, mFMM, fmmopts, buf),
                    (grad_stor, rs) -> optimize_radius_g!(grad_stor, rs, last_rs, shared_var, φs, α_inc, H, points, P, θ_i, Ns, k0, kin, centers, scatteringMatrices, dS_S, ids, mFMM, fmmopts, buf, minimize))
    end
    ...
    #         x_tol = optimopts.x_tol, f_tol = optimopts.f_tol, g_tol = optimopts.g_tol,
    #         store_trace = optimopts.store_trace, show_trace = optimopts.show_trace,
    #         extended_trace = optimopts.extended_trace,
    #         linesearch = method.linesearch!, iterations = outer_iterations,
    #         optimizer_o = Optim.Options(iterations = inner_iterations))
    optimize(df, initial_rs, r_min, r_max, Fminbox{LBFGS}();
